...
const queueService = {
  ...
  async startReview(ulanganId, gradeFunction = null, delayOverride = null) {
    ...
    // Jalankan pemrosesan secara asinkron di latar belakang
    const runWorker = async () => {
      try {
        while (!workerState.shouldStop) {
          // FR-10: Ambil 1 pekerjaan antrean tertua
          ...
          try {
            // Panggil fungsi penilai (Gemini atau custom grader)
            let result;
            if (gradeFunction) {
          ...
          // Cek apakah guru meminta berhenti
          if (workerState.shouldStop) break;

          // FR-11 & NFR-03: Jeda acak 5-13 detik antar-request
